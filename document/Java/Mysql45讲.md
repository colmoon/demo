...
1. 对于可重复读，查询只承认在事务启动前就已经提交完成的数据； 更新数据都是先读后写的，而这个读，只能读当前的值，称为“当前读”（current read）；除了更新，查询语句如果加锁，也是当前读，共享锁（lock in share mode），排他锁（for update，在mysql中，查询条件没用索引/主键的话就是表锁，否则就是是行锁。）。
...
介绍的WAL（Write-Ahead Logging ，预写日志系统，先写日志，再写磁盘）的概念，和你解释了这个机制后续需要的刷脏页操作 和执行时机。利用WAL技术，数据库将随机写转换成了顺序写，大大提升了数据库的性能。 但是，由此也带来了内存脏页的问题。脏页会被后台线程自动flush，也会由于数据页淘汰而触 发flush，而刷脏页的过程由于会占用资源，可能会让你的更新和查询语句的响应时间长一些。 在文章里，我也给你介绍了控制刷脏页的方法和对应的监控方式。
...
![image-20220816090849386](Mysql45讲.assets/image-20220816090849386.png)



就一起分析一下在两阶段提交的不同时刻，MySQL异常重启会出现什么现象。 
...
到这里，大家都可以理解。 大家出现问题的地方，主要集中在时刻B，也就是binlog写完，redo log还没commit前发生 crash，那崩溃恢复的时候MySQL会怎么处理？ 我们先来看一下崩溃恢复时的判断规则。 
...
2. 如果redo log里面的事务只有完整的prepare，则判断对应的事务binlog是否存在并完整： 
   ...
   ​	b. 否则，回滚事务。 这里，时刻B发生crash对应的就是2(a)的情况，崩溃恢复过程中事务会被提交；

事务binlog是有完整的格式的，可以通过此格式验证。
...
间隙锁和next-key lock的引入（间隙锁和行锁合称next-key lock），帮我们解决了幻读的问题，但同时也带来了一些“困扰”。

跟间隙锁存在冲突关系的，是“往这个间隙中插入一个记录”这个操 作。
...
2.  原则2：查找过程中访问到的对象才会加锁，有行才加行锁，没有命中那就加 next-key-lock 。
...
在实际开发中，我们也要尽量避免一些低效的方法，比如避免大量地使用短连接。同时，如果你 做业务开发的话，要知道，连接异常断开是常有的事，你的代码里要有正确地重连并重试的机制。



### 23 MySQL 是怎么保证数据不丢的

MySQL的“双1”配置，指的就是sync_binlog和innodb_flush_log_at_trx_commit都设 置成 1。也就是说，一个事务完整提交前，需要等待两次刷盘，一次是redo log（prepare 阶 段），一次是binlog。
...
![image-20220824110320744](Mysql45讲.assets/image-20220824110320744.png)



biglog三种格式，statement，row，mixed
...
2. 之后传给备库B，把备库接收完这个binlog时刻记为T2；
3. 备库执行完这个事务，这个时刻记为T；

seconds_behind_master ，即位 T3 - T1；

主库延迟的主要原因：备库性能差，备库压力大，大事务。

一次性地用delete语句删除太多数据，这就是一个典型的大事务场景。

**主备切换策略**：

可靠性优先策略，在双M结构下，切换过程如下：
